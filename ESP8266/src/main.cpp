...
#define ON_TEMPERATURE 26
#define ON_RATE 10
#define OFF_TEMPERATURE 24
#define MINIMUM_MINUTES 5
...
const char mqttClientId[] = "shower";
...
void fanOn() {
  ...
  if (!mqttClient.publish("shower/extractor", 1, true, "on")) {
    ESP.restart();
  }
}
...
void fanOff() {
  ...
  if (!mqttClient.publish("shower/extractor", 1, true, "off")) {
    ESP.restart();
  }
}
...
void loop() {
  ...
  if (now - lastMsg > 5000) {
    ...
    if (!mqttClient.publish("shower/rate", 1, true, msg)) {
      ESP.restart();
    }

    snprintf(msg, MSG_BUFFER_SIZE, "%0.1f", tempC);
    if (!mqttClient.publish("shower/temperature", 1, true, msg)) {
      ESP.restart();
    }
    ...
    switch (state) {
      ...
      case STATE_INITIAL:
        if (!mqttClient.publish("shower/state", 1, true, "initial")) {
          ESP.restart();
        }
        ...
        if ((millis() - fanOnTime) > 1000 * 60 * MINIMUM_MINUTES) {
          state = STATE_WAITING;
        }
      ...
      case STATE_WAITING:
        if (!mqttClient.publish("shower/state", 1, true, "waiting")) {
          ESP.restart();
        }
